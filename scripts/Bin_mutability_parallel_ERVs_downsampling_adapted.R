...
if (args[5] == "CRG36") {filter = "CRG36"} 
	...
	# if (args[5] == "CRG36_maskExons" | args[5] == "CRG36_Exons") {filter = "CRG36"} 

mutations_dataset = args[4]
mutation_matrix = args[4]

# if (include_CpGs == "no")   {downsampling = 218656/145743772}
if (include_CpGs == "no")   {downsampling = 218311/145538875}
if (include_CpGs == "only") {downsampling = 43806/10388103}
# if (include_CpGs == "only") {downsampling = 43869/10392665}


### Files ###
transcript_mutations <- fread(paste0("../../../mcortes/projects/mutable_TSS_regs/results/BEDS_otherdatasets/",CHR,"_",mutations_dataset,"_mutations_transcripts_and_co_FANTOM5_", args[5], "_strand5mer_CpG.txt"), header = T) 
...
transcript_mutations = sample_n(transcript_mutations, round(nrow(transcript_mutations)*downsampling), replace=FALSE)
...
all_sites50kb <- fread(paste0("../../../mcortes/projects/mutable_TSS_regs/results/BEDS_otherdatasets/", args[1], "_transcripts_and_co_FANTOM5_", args[5], "_strand5mer.txt"), header = F) 
...
if (args[3] == 10) {
	...
	transcript_mutations = subset(transcript_mutations, bin1000 <= 1)
	all_sites50kb = subset(all_sites50kb, bin1000 <= 1)
}


# this is more memory efficient
all_sites50kb$CpG <- rep("No", nrow(all_sites50kb))
all_sites50kb$CpG[all_sites50kb$strand3mer %in% CpG$strand3mer] <- "Yes"
...
# Mutation matrix
preglobal_mutation_matrix = fread(paste0("../../../mcortes/projects/mutable_TSS_regs/results/Mutation_Matrices/", mutation_matrix,"_", filter, ".txt"), sep = "\t", header = F) 
...
# mike comment: this is probably bettwer than last line...
preglobal_mutation_matrix <- preglobal_mutation_matrix[!grepl("N", strand_kmer)]
# nrow(preglobal_mutation_matrix)
...
for (i in 1:nrow(sdf)) {
	...
	tmpdt <- all_sites50kb[, list("X_Y_mutability" = sum(X_Y_mutability)), by = c("chr", "geneID", "position", "window_rank")]
	...
	if (i == 1) {
		...
		xydt[tmpdt, "X_Y_mutability" := i.X_Y_mutability, on = c("chr", "geneID", "position", "window_rank")]
...
for (x in c("A", "C", "G", "T")) {
	...
	tmpdt <- all_sites50kb[, list("X_content" = sum(X_content) / 3L), by = c("chr", "geneID", "position", "window_rank")]
	
	xydt[tmpdt, "X_content" := i.X_content, on = c("chr", "geneID", "position", "window_rank")]
...
write.table(sites_mutations, file = paste0("../results/Expected_mutations_per_bin/Chr_parallel_downsampled/",CHR,"_",mutations_dataset,"_obs_and_exp_mutations_",args[3],"_bin_size_",args[2],"_CpGs_", args[5],"_downsampled.txt"), col.names = F, row.names = F, quote = F, sep = "\t", append = F)
