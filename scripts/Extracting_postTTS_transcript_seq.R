...
all_chrs$size = all_chrs$`eedb:end` - all_chrs$`eedb:start.0base`
# hist(all_chrs$size)

#all_chrs = subset(all_chrs, size <= 50000)
...
# {
  ...
  #i
    ...
    if(row$`eedb:strand` == "+") {
      results = data.frame(seq(row$`eedb:end` + 1, row$`eedb:end` + 50000, 1))
      ...
      results1 = select(results, "chr", "start", "stop", "transcript", "geneID", "strand", "bin1000", "bin100", "bin10", "size")
      write.table(results1, file = paste0("../results/BEDS/", CHR, "_transcripts_all_postTTS_FANTOM5.bed"), quote = F, row.names = F, col.names = F, sep = "\t", append = T)
    }    
  ...
  #i = 529
    ...
    if(row$`eedb:strand` == "-") {
      ...
      results = data.frame(seq(row$`eedb:start.0base` - 1, row$`eedb:start.0base` - 50000, -1))
      colnames(results) <- c("position")
      results$chr = row$`eedb:chrom`
      results$start = results$position - 1
      results$stop = results$position + 0
