...
postTTS = fread(paste0("../results/BEDS/", CHR, "_transcripts_all_postTTS_nontranscribed_FANTOM5.bed"), header = F)
## for postTTS rmv sites transcribed

upstream = fread(paste0("../results/BEDS/", CHR, "_transcripts_upstream_nontranscribed_FANTOM5.bed"), header = F)
## for upstream rmv sites transcribed

downstream = fread(paste0("../results/BEDS/", CHR, "_transcripts_downstream_nontranscribed_FANTOM5.bed"), header = F)
## for downstream rmv sites > 50000 and sites transcribed by more than one transcript

upstream_TTS = fread(paste0("../results/BEDS/", CHR, "_transcripts_upstream_TTS_nontranscribed_FANTOM5.bed"), header = F)
...
postTTS$V11 = "downstream_TTS"
upstream$V11 = "upstream_TSS"
downstream$V11 = "downstream_TSS"
upstream_TTS$V11 = "upstream_TTS"

# nrow(inner_join(postTTS_3, upstream_3, by = c("V2", "V3")))

upstream_postTTS = data.frame(rbind(upstream, postTTS))
rm(list=c("upstream", "postTTS"))
...
rm(list=c("upstream_postTTS"))

downstream_upstream_TTS = data.frame(rbind(downstream, upstream_TTS))
rm(list=c("downstream", "upstream_TTS"))
downstream_upstream_TTS = downstream_upstream_TTS[order(downstream_upstream_TTS$V3, downstream_upstream_TTS$V9),] #take the sites closest to the nearest TSS or TTS
downstream_upstream_TTS_2 = downstream_upstream_TTS[!duplicated(downstream_upstream_TTS$V3),] #there are sites that are at the same distance in different transcripts but I just take a random one
rm(list=c("downstream_upstream_TTS"))

upstream_postTTS_downstream = rbind(upstream_postTTS_2, downstream_upstream_TTS_2)
rm(list=c("upstream_postTTS_2", "downstream_upstream_TTS_2"))
upstream_postTTS_downstream = upstream_postTTS_downstream[order(upstream_postTTS_downstream$V3),] 
upstream_postTTS_downstream = subset(upstream_postTTS_downstream, V2 > 0)
...
write.table(upstream_postTTS_downstream, file = paste0("../results/BEDS/", CHR, "_transcripts_and_co_FANTOM5_all.bed"), quote = F, row.names = F, col.names = F, sep = "\t", append = F)
